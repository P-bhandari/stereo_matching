...
namespace sparsestereo {
	...
	public:	
		...
		void match(const cv::Mat& left, const cv::Mat& right, const std::vector<cv::KeyPoint>& leftFeat,
			const std::vector<cv::KeyPoint>& rightFeat, std::vector<SparseMatch>* matches, int combination);
	...
	private:
		...
		void calcCosts(const cv::Mat& left, const cv::Mat& right, unsigned int* rowOffsets, int combination);
		cv::Point2f subPixelTestFunction(CORRELATION correlation,const cv::Mat& left, const cv::Mat& right, int combination, cv::Point2f referencePoint,cv::Point2f rightFeature, float cost) ;

		// Performs a left/right consistency check that is dense in the left image
		void denseConsistencyCheck(const cv::Mat& left, const cv::Mat& right, int combination);
	};
}
