...
Mat postProcessingAndComparing(Mat disp);
...
int main(int argc, char** argv) {
	...
	try {
		...
		string RightpathName ="/home/piyush/Documents/NTU Internship/stereo matching/SparseSetero Code/Baselines/training/";
		 ...
		 double matchesAfterDense = 0 ;
		 double inputImageSize = 0 ;
		// Read input images
		for(lexer = 0 ; lexer <193;lexer++)
		 {
			...
			leftImg = imread(LeftpathName+"image_0/"+filename+fileType, CV_LOAD_IMAGE_GRAYSCALE);
			rightImg = imread(RightpathName+"image_1/"+filename+fileType, CV_LOAD_IMAGE_GRAYSCALE);
			leftColoredImg = imread(LeftpathName+"colored_0/"+filename+fileType, CV_LOAD_IMAGE_COLOR);
			rightColoredImg = imread(RightpathName+"colored_1/"+filename+fileType, CV_LOAD_IMAGE_COLOR);
    	...
    	int width, height;
    	width = leftColoredImg.size().width;
        height = leftColoredImg.size().height;
		...
		// The stereo matcher. SSE Optimized implementation is only available for a 5x5 window
			SparseStereo<CensusWindow<5>, long double> stereo(maxDisp, 1, uniqueness,
		...
		// For performance evaluation we do the stereo matching 100 times
			...
			if(sparse == 0){
				...
				finalDisparityMap = postProcessingAndComparing(disparityImage);
				//finalDisparityMap = disparityImage;
				 ...
				 if(edgeNo == 5){
				    	...
				    	imwrite("/home/piyush/Downloads/original exFast/Image outputs/KITTI Disp/"+filename+fileType, disparityImage, compression_params);
				    }
				if(halfImage == 1)
			...
			cv::Mat_<unsigned char> leftSGBMImg, rightSGBMImg;

			leftSGBMImg = imread(LeftpathName+"image_0/"+filename+fileType, CV_LOAD_IMAGE_GRAYSCALE);
			rightSGBMImg = imread(RightpathName+"image_1/"+filename+fileType, CV_LOAD_IMAGE_GRAYSCALE);
			
			Mat SGBMDisp = disparityMapOfSGBM(leftSGBMImg,rightSGBMImg); 	//to read the values use the following function(for cv_16U type values)	 (float)disp8.at<short>(i,j)/16.0f
			time_duration elapsed2 = (microsec_clock::local_time() - lastTime2);
			cout << "Time for SGBM matching: " << elapsed2.total_microseconds()/1.0e6 << "s" << endl;	//to read the values use the following function(for cv_16U type values)	 (float)disp8.at<short>(i,j)/16.0f
...
void sparseToDenseConversion(Mat leftColoredImg, Mat rightColoredImg, Mat& disparityImage,Mat labels,Mat leftMask, vector<SparseMatch> correspondences, int edgeNo)
{
   ...
   for(int i =0 ; i<rows;i++)	//y
	...
	// imshow("Disparity", disparityImage/maxDisp);
	// waitKey();
...
void nonoccGTCompare(Mat disp, vector<SparseMatch> correspondences, int lexer, string fileType, string filename, int censusWinType, int combination)
{
	...
	for(int i=0;i<GT.rows;i++)
	{
		...
		{
		//	cout<<" i , j = "<<i <<", "<<j<<endl;
			if((float)(GT.at<short>(i,j)) ==0 )
				invalidPoint++;
			else
			{
				float x = abs((float)disp.at<float>(i,j)- (float)(GT.at<short>(i,j)/(2*256.0)));
				if(x<=0.5)
				{
						withinErrorHalf++;
				}
...
void nonoccGTCompareSparse( vector<SparseMatch> correspondences, int lexer, string fileType, string filename, int censusWinType, int combination)
{
	...
	for(int i =0  ;i<correspondences.size();i++)
	{
		...
		{
				float x = abs( correspondences.at(i).disparity() - (float)(GT.at<short>(correspondences[i].rectLeft.y,correspondences[i].rectLeft.x)/256.0));
				if(x<=0.5)
				{
					withinErrorHalf++;
			...
			{	
			 ...
			 NOCaverageForSGBMWithinErrorHalf[k][i]=0 ; 
				...
				for(int j=0;j<lexer;j++)
				{
					 ...
					 cout<<"OccAverage = "<< OCCaverageForCannyEdgeWithinError1[k][i]<<endl;
...
for(int i=0;i<GT.rows;i++)
	for(int j=0;j<(int)GT.cols;j++)
	{
		...
		{
			float x = abs((SGBMDisp.at<short>(i,j)/16.0f)- (float)(GT.at<short>(i,j)/256.0));
			if(x<=0.5)
				withinErrorHalf++;
			else if(x <=1)
				withinError1++;
...
void occSgbmGTCompareSparse(vector<SparseMatch> correspondences, int lexer, string fileType,string filename, Mat SGBMDisp,int censusWinType,int combination)
{
	...
	for(int i=0;i<(int)correspondences.size();i++)
	{
		...
		{
			float x = abs((float)SGBMDisp.at<short>(correspondences[i].rectLeft.y,correspondences[i].rectLeft.x)/16.0f- (float)(GT.at<short>(correspondences[i].rectLeft.y,correspondences[i].rectLeft.x)/256.0));
...
for(int i=0;i<GT.rows;i++)
	for(int j=0;j<(int)GT.cols;j++)
	{
		...
		else
		{
			...
			float x = abs((float)SGBMDisp.at<short>(i,j)/16.0f- (float)(GT.at<short>(i,j)/256.0));
			if(x<=0.5)
				withinErrorHalf++;
			else if(x <=1 )
				withinError1++;
